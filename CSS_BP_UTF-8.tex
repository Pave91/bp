...
\newacronym{DSL}{DSL}{(Dynamic Stylesheet language)}
\newacronym{IE}{IE}{(Internet Explorer)}
\newacronym{UI}{UI}{(User interface)}
\newacronym{YAML}{YAML}{(YAML Ain't Markup Language)}
\newacronym{PHP}{PHP}{(PHP: Hypertext Preprocessor) znamená hypertextový preprocesor. Jedná se o~skriptovací jazyk, který je používán k vývoji webových stránek. PHP se provádí na straně serveru}
\newacronym{Sass}{Sass}{(Syntactically Awesome Stylesheets)}
\newacronym{CMS}{CMS}{(Content Management System) slouží pro správu webového obsahu}
